...
import okaiyi.mockingjay.json.parse.JsonParseUtil;
...
public abstract class WeixinResponse {
	...
	protected Map<String,DataTyper> responseResult;
	public WeixinResponse(String msg){
		//解析微信数据
		responseResult=doParser(msg);
	}
	/**
	 * 解析微信消息,默认使用json方式解析,如果有其他方式,子类需要重写该方法
	 * @return
	 */
	protected  Map<String,DataTyper> doParser(String msg){
		return JsonParseUtil.parser(msg);
	}
	/**
	 * 获取指定消息的结果
	 * @param key
	 * @return
	 */
	protected String getDataTypeToString(String key){
		DataTyper data=responseResult.get(key);
		return data==null?"":data.getString();
	}
	...
	public boolean isError(){
		return responseResult.get(ERR_CODE)!=null;
	}
	/**
	 * 获取错误编码,如果没有错误,返回空字符
	 * @return
	...
	public String getErrorCode(){
		return getDataTypeToString(ERR_CODE);
	}
	/**
	 * 获取错误信息
	 * @return
	...
	public String getErrorMsg(){
		return getDataTypeToString(ERR_MSG);
	}
	/**
	 * 获取错误编码的中文提示
	 * @param errorCode
	 * @return
	 */
	public static final String errorCodeToChinese(String errorCode){
	...
	static{
		...
		errorCodeMap.put("40013","不合法的AppID，请开发者检查AppID的正确性，避免异常字符，注意大小写");
		errorCodeMap.put("40014","不合法的access_token，请开发者认真比对access_token的有效性（如是否过期），或查看是否正在为恰当的公众号调用接口");
		...
		errorCodeMap.put("42001","access_token超时，请检查access_token的有效期，请参考基础支持-获取access_token中，对access_token的详细机制说明");
